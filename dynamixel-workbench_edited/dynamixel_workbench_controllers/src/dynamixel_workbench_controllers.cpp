...
void DynamixelController::initSubscriber()
{
  ...
  position_sub = priv_node_handle_.subscribe("motor_pos_command", 100, &DynamixelController::motorpositionCallback, this);
...
void DynamixelController::motorpositionCallback(const std_msgs::Int32MultiArray::ConstPtr &array)
{
  bool result = false;
  const char* log;
  ...
  int32_t target_values[dynamixel_.size()] = {};
  ...
  {
    target_values[i] = array->data[i];
  }

  result = dxl_wb_->syncWrite(SYNC_WRITE_HANDLER_FOR_GOAL_VELOCITY, id_array, dynamixel_.size(), target_values, 1, &log);
  if (result == false)
  {
    ROS_ERROR("%s", log);
  }
